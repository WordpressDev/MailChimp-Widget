	...
	public function widget ($args, $instance) {
		...
		if ((isset($_COOKIE[$this->id_base . '-' . $this->number]) && $this->hash_mailing_list_id($this->number) == $_COOKIE[$this->id_base . '-' . $this->number]) || false == $this->ns_mc_plugin->get_mcapi()) {
			...
			if ($this->successful_signup) {
				...
				$widget .= '<form class="mailchimp_submit" action="' . $_SERVER['REQUEST_URI'] . '" id="' . $this->id_base . '_form-' . $this->number . '" method="post">';
				$widget .= $this->subscribe_errors . $collect_first . $collect_last . '';
				$widget .= '<label>' . __('Email Address :', 'mailchimp-widget') . '</label>';
				$widget .= '<input type="hidden" name="ns_mc_number" value="' . $this->number . '" />';
				$widget .= '<input type="text" name="' . $this->id_base . '_email" />';
				$widget .= '<input class="button" type="submit" name="' . __($instance['signup_text'], 'mailchimp-widget') . '" value="' . __($instance['signup_text'], 'mailchimp-widget') . '" />';
				$widget .= '</form>';
				$widget .= '<script type="text/javascript"> jQuery(\'#' . $this->id_base . '_form-' . $this->number . '\').ns_mc_widget({"url" : "' . $_SERVER['PHP_SELF'] . '", "cookie_id" : "'. $this->id_base . '-' . $this->number . '", "cookie_value" : "' . $this->hash_mailing_list_id() . '", "loader_graphic" : "' . $this->default_loader_graphic . '"}); </script>';
